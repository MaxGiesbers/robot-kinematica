...
{
std::optional<Matrix<double, 3, 1>> Kinematics::forward_kinematics(
    Matrix<double, 4, 1>& angles) const
{
	...
	// x axis
	fk_solution[0][0] = ((l1 * std::sin(a[1][0])) + (l2 * std::sin(a[1][0] + a[2][0]))
	                     + (l3 * std::sin(a[1][0] + a[2][0] + a[3][0])))
	                    * std::cos(a[0][0]);

	// y axis
	fk_solution[1][0] = (l1 * std::cos(a[1][0])) + (l2 * std::cos(a[1][0] + a[2][0]))
	                    + (l3 * std::cos(a[1][0] + a[2][0] + a[3][0]));

	// z axis
	fk_solution[2][0] = ((l1 * std::sin(a[1][0])) + (l2 * std::sin(a[1][0] + a[2][0]))
	                     + (l3 * std::sin(a[1][0] + a[2][0] + a[3][0])))
	                    * std::sin(a[0][0]);
...
std::optional<Matrix<double, 4, 1>> Kinematics::inverse_kinematics(
    Matrix<double, 4, 1>& current_angles,
...
Matrix<double, 3, 3> Kinematics::compute_jacobian(const Matrix<double, 4, 1>& angles) const
{
	...
	// x axis
	jacobian_solution[0][0] =
	    (l1 * std::sin(a[1][0]) + l2 * std::sin(a[1][0] + a[2][0])) * -std::sin(a[0][0]);
	jacobian_solution[0][1] =
	    (l1 * std::cos(a[1][0]) + l2 * std::cos(a[1][0] + a[2][0])) * std::cos(a[0][0]);
	...
	// z axis
	jacobian_solution[2][0] =
	    (l1 * std::sin(a[1][0]) + l2 * std::sin(a[1][0] + a[2][0])) * std::cos(a[0][0]);
	jacobian_solution[2][1] =
	    (l1 * std::cos(a[1][0]) + l2 * std::cos(a[1][0] + a[2][0])) * std::sin(a[0][0]);
