...
class ObjectDetector
{
    ...
    void filterColor(std::shared_ptr<ColorObject>& color_object, cv::Mat& filtered_frame);
    void DrawImageContours(const std::vector<std::vector<cv::Point>> &contour, 
    const std::shared_ptr<ColorObject>& color_object, const int contour_number, cv::Mat& drawing_frame);
